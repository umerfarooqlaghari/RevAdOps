...
// Bulk update content for any page (POST method for frontend compatibility)
router.post('/bulk-update', authenticateToken, adminAuth, async (req, res) => {
  try {
    const { updates } = req.body;
    const results = [];
    ...
    for (const update of updates) {
      ...
      try {
        // For homepage sections, add homepage_ prefix, for others use as-is
        const sectionName = section.startsWith('homepage_') ? section :
                           ['hero', 'what_we_do', 'why_choose_us', 'how_it_works', 'our_expertise', 'testimonials', 'partners', 'final_cta', 'header', 'footer'].includes(section)
                           ? `homepage_${section}` : section;
...
// Homepage bulk update endpoint (POST method for frontend compatibility)
router.post('/homepage/bulk-update', authenticateToken, adminAuth, async (req, res) => {
  try {
    const { updates } = req.body;
    const results = [];
