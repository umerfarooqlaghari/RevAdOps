...
const DynamicHeroSection = ({ content }: DynamicHeroSectionProps) => {
  ...
  const defaultContent = {
    ...
    background_image: "https://images.unsplash.com/photo-1518709268805-4e9042af2176?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2025&q=80",
  ...
  // Create background style with proper fallback
  const backgroundStyle = heroData.background_image
    ? {
  ...
  return (
    ...
    >
      {/* Fallback gradient background with dark overlay if no image */}
      ...
      {/* Subtle overlay for text readability - only if background image exists */}
      {heroData.background_image && (
